...
This is a simple dashboard to illustrate the use of the [`njoaguof`](https://github.com/tor-gu/njoaguof) data package for R, which is a
repackaging of the the use of force data released by the [New Jersey 
Office of the Attorney General](https://www.njoag.gov). 


The NJ OAG has their own [dashboard](https://www.njoag.gov/force/). If your
interest is in gathering summary data for this dataset, you should start there.

## About the data
### Current version
This site is based on `njoaguof` version `r packageVersion("njoaguof")`, which contains
data from `r min(njoaguof::incident$incident_date_1)` through `r max(njoaguof::incident$incident_date_1)`.
...
* A single use of force report may refer to more than one subject of the use of force, and multiple types of force for each subject.
* Multiple officers may file separate reports for the same incident.
* Reports from different officers for the same incident may refer to exactly the same subjects, to different subjects, or to partially overlapping sets of subjects.

Because a single incident report may contain multiple subjects, the subjects are broken out into a separate table.

Unfortunately, the underlying dataset does not allow us to disentagle these potentially overlapping `incident`s and `subject`s.  Thus, this application counts every `incident` and every `subject` *separately*. This means that the `incident` and `subject` counts are typically *overcounts*. (The [NJ OAG dashboard](https://www.njoag.gov/force/) makes the same decision.) However, see the special note on filtering [multi-value `subject` fields](#subjectmulti), below.
...
selections for each subject -- for example, an incident report with multiple subjects may contain the the `force_type` value "Used pressure points on" twice, since it applies to two subjects in the incident report.
...
The maps are TIGER/Line 2024 boundaries from the US Census, obtained through the [tigris](https://github.com/walkerke/tigris) package. The population figures are the US Census Population Estimates Program's vintage 2024 estimates.

The source code is available [here](https://github.com/tor-gu/njoaguofdash).